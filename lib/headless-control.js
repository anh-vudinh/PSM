    ...
    switch (action) {
        ...
        case "graceful-stop": {
            ...
            if (
                ...
                throw new Error(
                    "Graceful stop waittime must be at least 1 second"
                );
            }

            if (!message) {
                throw new Error(
                    "Graceful stop message is empty"
                );
            }

            async function gracefulStopWorld(
                world
            ) {
                if (
                    !sup.isRunning(
                ...
                dbm.logEvent(
                    world.world_id,
                    "graceful-stop",
                    `Graceful shutdown scheduled in ${waittime}s: ${message}`
                );
            ...
            return await gracefulStopWorld(
                world
            );
        }
